        ...
        for row in tqdm(openai_df.iloc()):
            ...
            if batch_details.status == 'completed':
                ...
                for idx, item in enumerate(json_output):
                    ...
                    original_details = data_df.loc[(data_df['url'] == url) & (data_df['role'] == role) & (data_df['role_sequence'] == role_sequence) & (data_df['round'] == int(round))]
                    assert len(original_details) == 1
                    formatted_output['url'] = url
                    formatted_output['role'] = role
                    formatted_output['claim'] = original_details['generated_claim']
                    formatted_output['intent'] = original_details['generated_intent']
                    formatted_output['label'] = None

                    formatted_output['question_key'] = question_key
                    formatted_output['answer'] = None
                    if question_key != 'Q5':
                        formatted_output['answer'] = current_response['Answer']
                        formatted_output['new_label'] = None
                        formatted_output['explanation'] = current_response['Explanation']
                    elif question_key == 'Q5':
                        formatted_output['new_label'] = current_response['New Label'].lower()
                        if 'Explanation' in current_response:
                            formatted_output['explanation'] = current_response['Explanation']
                        else:
                            print(current_response)
                            formatted_output['explanation'] = current_response['Confidence Scores']['Explanation']
